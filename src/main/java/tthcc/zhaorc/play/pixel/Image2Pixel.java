...
public class Image2Pixel {
     ...
     * @throws Exception
     */
    private void makePixelFile(PixelConfig piexlConfig, List<Color> colorList) throws Exception {
        int idx = piexlConfig.getFilename().lastIndexOf("/");
        String filename = piexlConfig.getFilename().substring(idx);
        filename = piexlConfig.getFilename().replace(filename, "/web" + filename);
        ...
        lineList.addAll(piexlConfig.getHeaderLineList());
        lineList.add("<body>");
        lineList.add(String.format(piexlConfig.getPixelTableTpl(), piexlConfig.getBlocksX() > piexlConfig.getBlocksY() ? "pixel_table_w" : "pixel_table_h"));
        lineList.add("<tr>");
        for (int i = 0; i < colorList.size(); i++) {
            if (i > 0 && i % piexlConfig.getBlocksX() == 0) {
                lineList.add("</tr>");
                lineList.add("<tr>");
            }
            lineList.add(String.format(piexlConfig.getPixelTdTpl(), colorList.get(i).getName()));
        }
        lineList.add("</tr>");
        lineList.add("</table>");
        lineList.add("</body>");
            ...
            for (int col = 0; col < cols; col++) {
                ...
                for (int y = 0; y < pixelConfig.getPaperBlocks(); y++) {
                    ...
                    for (int x = 0; x < pixelConfig.getPaperBlocks(); x++) {
                        ...
                        paperLineList.add(String.format(pixelConfig.getPixelTdTpl(), colorList.get(offset).getName()));
                    }
                }
                paperLineList.add("</tr>");
    ...
    private String makePaperPartsBoxLine(PixelConfig pixelConfig, List<Color> paperColorList) {
        if (pixelConfig.getPartsBoxX() == 0) {
            return "<script>var partsBoxMap = {};</script>";
        }
